...
function AllCalendar({ month }: { month: Date }) {
  ...
  return <div className="p-4 sm:p-6"><div className="mb-4 flex flex-wrap gap-3 text-xs">{REQUEST_TYPES.map((type) => <span key={type} className="text-zinc-500"><i className="mr-1 inline-block h-2.5 w-2.5 rounded-full" style={{ backgroundColor: TYPE_COLORS[type] }} />{TYPE_LABELS[type]}</span>)}<span className="text-zinc-500"><i className="mr-1 inline-block h-2.5 w-2.5 rounded-full bg-orange-600" />Dépôt</span><span className="text-zinc-500"><i className="mr-1 inline-block h-2.5 w-2.5 rounded-full bg-lime-600" />Évènement</span></div><div className="overflow-x-auto rounded-2xl border border-[var(--crm-border)] bg-[var(--crm-surface)]"><div className="min-w-[720px]"><WeekdayHeader /><div className="grid grid-cols-7">{useMonthDays(month).map((day) => <DayCell key={day.toISOString()} day={day} inMonth={isSameMonth(day, month)} isSelected={false} onClick={() => {}}>{(requests ?? []).filter((item) => item.scheduledDate && isSameDay(new Date(item.scheduledDate), day)).map((item) => <div key={item._id} className="mb-1 truncate rounded-md px-1.5 py-1 text-[11px] font-medium text-white" style={{ backgroundColor: TYPE_COLORS[item.type] }}>{item.customer.lastName} · {TYPE_LABELS[item.type]}</div>)}{(depots ?? []).filter((item) => item.depot && isSameDay(new Date(item.depot.slotStart), day)).map((item) => <div key={item._id} className="mb-1 truncate rounded-md bg-orange-600 px-1.5 py-1 text-[11px] font-medium text-white">{format(new Date(item.depot!.slotStart), "HH:mm")} · Dépôt {item.depot!.site}</div>)}{(events ?? []).filter((item) => isSameDay(new Date(item.startAt), day)).map((item) => <div key={item._id} className="mb-1 truncate rounded-md bg-lime-600 px-1.5 py-1 text-[11px] font-medium text-white">{format(new Date(item.startAt), "HH:mm")} · {item.title}</div>)}</DayCell>)}</div></div></div></div>;
}
