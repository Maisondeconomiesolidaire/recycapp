...
function AllCalendar({ month }: { month: Date }) {
  ...
  const events = useQuery((api as any).recycappCalendar.list, range) as Array<{ _id: string; title: string; startAt: number; endAt: number; urls?: string[]; attachmentUrls?: Array<string | null> }> | undefined;
  const [detail, setDetail] = useState<{ title: string; content: ReactNode } | null>(null);
  return <div className="p-4 sm:p-6"><div className="mb-4 flex flex-wrap gap-3 text-xs">{REQUEST_TYPES.map((type) => <span key={type} className="text-zinc-500"><i className="mr-1 inline-block h-2.5 w-2.5 rounded-full" style={{ backgroundColor: TYPE_COLORS[type] }} />{TYPE_LABELS[type]}</span>)}<span className="text-zinc-500"><i className="mr-1 inline-block h-2.5 w-2.5 rounded-full bg-orange-600" />Dépôt</span><span className="text-zinc-500"><i className="mr-1 inline-block h-2.5 w-2.5 rounded-full bg-lime-600" />Évènement</span></div><div className="overflow-x-auto rounded-2xl border border-[var(--crm-border)] bg-[var(--crm-surface)]"><div className="min-w-[720px]"><WeekdayHeader /><div className="grid grid-cols-7">{useMonthDays(month).map((day) => <DayCell key={day.toISOString()} day={day} inMonth={isSameMonth(day, month)} isSelected={false} onClick={() => {}}>{(requests ?? []).filter((item) => item.scheduledDate && isSameDay(new Date(item.scheduledDate), day)).map((item) => <button key={item._id} onClick={() => setDetail({ title: "Demande", content: <><p className="font-semibold">{item.customer.firstName} {item.customer.lastName}</p><p>{TYPE_LABELS[item.type]}</p><p>{item.customer.phone}</p></> })} className="mb-1 w-full truncate rounded-md px-1.5 py-1 text-left text-[11px] font-medium text-white" style={{ backgroundColor: TYPE_COLORS[item.type] }}>{item.customer.lastName} · {TYPE_LABELS[item.type]}</button>)}{(depots ?? []).filter((item) => item.depot && isSameDay(new Date(item.depot.slotStart), day)).map((item) => <button key={item._id} onClick={() => setDetail({ title: `Dépôt ${item.depot!.site}`, content: <><p className="font-semibold">{item.customer.firstName} {item.customer.lastName}</p><p>Créneau : {format(new Date(item.depot!.slotStart), "HH:mm")} – {format(new Date(item.depot!.slotEnd), "HH:mm")}</p></> })} className="mb-1 w-full truncate rounded-md bg-orange-600 px-1.5 py-1 text-left text-[11px] font-medium text-white">{format(new Date(item.depot!.slotStart), "HH:mm")} · Dépôt {item.depot!.site}</button>)}{(events ?? []).filter((item) => isSameDay(new Date(item.startAt), day)).map((item) => <button key={item._id} onClick={() => setDetail({ title: item.title, content: <><p>{format(new Date(item.startAt), "EEEE d MMMM HH:mm", { locale: fr })} – {format(new Date(item.endAt), "HH:mm")}</p>{item.urls?.map((url) => <a key={url} href={url} target="_blank" rel="noreferrer" className="block text-brand-600 underline">{url}</a>)}</> })} className="mb-1 w-full truncate rounded-md bg-lime-600 px-1.5 py-1 text-left text-[11px] font-medium text-white">{format(new Date(item.startAt), "HH:mm")} · {item.title}</button>)}</DayCell>)}</div></div></div><Modal open={!!detail} onClose={() => setDetail(null)} title={detail?.title ?? "Détail"}>{detail?.content}</Modal></div>;
}
...
function EventModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [title, setTitle] = useState(""); const [start, setStart] = useState<number>(); const [end, setEnd] = useState<number>(); const [urls, setUrls] = useState(""); const [file, setFile] = useState<File | null>(null);
  async function save() { const attachments = file ? [await upload(file)] : []; await create({ title, startAt: start!, endAt: end!, attachments, urls: urls.split(/\n|,/).map((url) => url.trim()).filter(Boolean) }); onClose(); }
  return <Modal open={open} onClose={onClose} title="Nouvel évènement"><div className="space-y-5"><div className="rounded-2xl border border-[var(--crm-border)] bg-[var(--crm-surface-2)] p-4 text-sm text-[var(--foreground)]">Planifiez un évènement, joignez ses documents et centralisez les liens utiles.</div><Field label="Intitulé" required><input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ex. Atelier réparation vélo" className="h-11 w-full rounded-xl border border-[var(--crm-border)] bg-[var(--crm-surface)] px-3 text-[var(--foreground)] shadow-sm outline-none focus:border-brand-500" /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Date et heure de début" required><DateTimePicker value={start} onChange={setStart} placeholder="Choisir le début" /></Field><Field label="Date et heure de fin" required><DateTimePicker value={end} onChange={setEnd} placeholder="Choisir la fin" /></Field></div><Field label="Pièce jointe"><label className="flex cursor-pointer items-center justify-between rounded-2xl border border-dashed border-brand-400 bg-[var(--crm-surface-2)] px-4 py-4 text-sm font-semibold text-brand-600 transition hover:bg-[var(--crm-surface)]"><span>{file ? file.name : "Ajouter un fichier"}</span><span className="rounded-lg bg-[var(--crm-surface)] px-3 py-1.5 text-xs shadow-sm">Parcourir</span><input type="file" className="hidden" onChange={(e) => setFile(e.target.files?.[0] ?? null)} /></label></Field><Field label="URLs"><textarea value={urls} onChange={(e) => setUrls(e.target.value)} className="min-h-24 w-full rounded-xl border border-[var(--crm-border)] bg-[var(--crm-surface)] p-3 text-[var(--foreground)] shadow-sm outline-none focus:border-brand-500" placeholder="Une URL par ligne" /></Field><div className="flex justify-end gap-2"><Button variant="outline" onClick={onClose}>Annuler</Button><Button onClick={() => void save()} disabled={!title || !start || !end}>Créer l'évènement</Button></div></div></Modal>;
}
...
export function ResourceCalendar({ siteFilter }: { siteFilter: Site | null }) {
  ...
  const allWorkers = useQuery(api.polyvalents.listWorkers, canRead ? {} : "skip");
  ...
  const workers = useMemo(
    () => (allWorkers ?? []).filter((worker) => !siteFilter || worker.sites?.includes(siteFilter)),
    [allWorkers, siteFilter],
  );
  const tasks = useMemo(
    () => (allTasks ?? []).filter((task) => !siteFilter || task.site === siteFilter),
    [allTasks, siteFilter],
  );
  ...
  const byDay = useMemo(() => {
    ...
    const matchesSite = (taskSite: Site | null | undefined) => !siteFilter || taskSite === siteFilter;
            ...
            return (
              <div
                ...
                onDrop={(event) => {
                  ...
                  const hour = Math.max(RESOURCE_DAY_START_HOUR, Math.min(RESOURCE_DAY_END_HOUR - 1, RESOURCE_DAY_START_HOUR + Math.floor((event.clientY - bounds.top) / RESOURCE_HOUR_HEIGHT)));
                  setDroppedTask({ taskId, startAt: dayAtHour(day, hour), endAt: dayAtHour(day, hour + 1) });
                  setSelectedDay(day);
                }}
                className={cn(
              ...
              >
                ...
                {items.map((activity) => {
                  ...
                  return <button key={activity._id} type="button" onClick={(event) => { event.stopPropagation(); setSelectedDay(day); }} className="absolute left-1 right-1 z-10 overflow-hidden rounded-md border border-brand-400/40 bg-brand-500/25 px-1.5 py-1 text-left text-[11px] font-medium text-brand-100 shadow-sm transition hover:bg-brand-500/40" style={{ top: segment.top, height: segment.height }} title={`${activity.workerName} — ${activity.taskName} · ${segment.timeLabel}`}>
                    <p className="flex items-center gap-1 truncate font-semibold"><span className="truncate">{activity.taskName}</span>{!activity.workerId ? <span title="Aucun salarié affecté" aria-label="Aucun salarié affecté"><TriangleAlert className="h-3.5 w-3.5 shrink-0 text-amber-300" /></span> : null}</p>
                    <p className="truncate text-brand-100/80">{activity.workerName}</p>
