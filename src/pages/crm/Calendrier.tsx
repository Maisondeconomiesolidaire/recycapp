...
function EventModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <Modal open={open} onClose={onClose} title="Nouvel évènement"><div className="space-y-5"><div className="rounded-2xl bg-brand-50 p-4 text-sm text-brand-800">Planifiez un évènement, joignez ses documents et centralisez les liens utiles.</div><Field label="Intitulé" required><input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ex. Atelier réparation vélo" className="h-11 w-full rounded-xl border border-border bg-input/40 px-3 shadow-sm outline-none focus:border-brand-500" /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Date et heure de début" required><DateTimePicker value={start} onChange={setStart} placeholder="Choisir le début" /></Field><Field label="Date et heure de fin" required><DateTimePicker value={end} onChange={setEnd} placeholder="Choisir la fin" /></Field></div><Field label="Pièce jointe"><label className="flex cursor-pointer items-center justify-between rounded-2xl border border-dashed border-brand-300 bg-brand-50/50 px-4 py-4 text-sm font-semibold text-brand-700 transition hover:bg-brand-50"><span>{file ? file.name : "Ajouter un fichier"}</span><span className="rounded-lg bg-white px-3 py-1.5 text-xs shadow-sm">Parcourir</span><input type="file" className="hidden" onChange={(e) => setFile(e.target.files?.[0] ?? null)} /></label></Field><Field label="URLs"><textarea value={urls} onChange={(e) => setUrls(e.target.value)} className="min-h-24 w-full rounded-xl border border-border bg-input/40 p-3 shadow-sm outline-none focus:border-brand-500" placeholder="Une URL par ligne" /></Field><div className="flex justify-end gap-2"><Button variant="outline" onClick={onClose}>Annuler</Button><Button onClick={() => void save()} disabled={!title || !start || !end}>Créer l'évènement</Button></div></div></Modal>;
}
