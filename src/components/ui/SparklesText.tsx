...
function Star({ color, scale }: { color: string; scale: number }) {
  return (
    <svg
      width="21"
      height="21"
      ...
      style={{ ["--spark-scale" as string]: scale } as CSSProperties}
    >
      <path
...
/**
 * Texte « étincelant » : des étoiles apparaissent en continu autour du texte,
...
export function SparklesText({
  ...
  useEffect(() => {
    ...
    setSparks(Array.from({ length: sparklesCount }, generate));

    const interval = setInterval(() => {
      setSparks((current) =>
        current.map((spark) =>
          spark.lifespan <= 0 ? generate() : { ...spark, lifespan: spark.lifespan - 0.1 },
        ),
      );
    }, 100);
  ...
  return (
    <span className={cn("relative inline-block", className)}>
      ...
      {sparks.map((spark) => (
        <span
          key={spark.id}
          className="pointer-events-none absolute z-20 h-[21px] w-[21px]"
          style={{
            left: spark.x,
            top: spark.y,
        ...
        >
          <Star color={spark.color} scale={spark.scale} />
        </span>
      ))}
