...
function buildPinsGeoJson(depot: [number, number] | null, stops: TrackingStop[]) {
  ...
  for (const s of stops) {
    ...
    features.push({
      ...
      properties: {
        kind: s.isRecipient ? "recipient" : s.done ? "done" : "pending",
...
export function LiveDeliveryTracking({ token }: { token: string }) {
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      ...
      map.addSource("pins", { type: "geojson", data: buildPinsGeoJson(depot, stops) });
      map.addLayer({
        id: "pins-circles",
        type: "circle",
        source: "pins",
        paint: {
          ...
          "circle-color": ["match", ["get", "kind"], "depot", "#0f766e", "recipient", "#ef4444", "done", "#16a34a", "#71717a"],
      ...
      map.addLayer({
        ...
        layout: { "text-field": ["get", "label"], "text-size": 12, "text-font": ["DIN Pro Bold", "Arial Unicode MS Bold"], "text-allow-overlap": true },
  ...
  useEffect(() => {
    ...
    (map.getSource("pins") as mapboxgl.GeoJSONSource | undefined)?.setData(buildPinsGeoJson(depot, stops));
