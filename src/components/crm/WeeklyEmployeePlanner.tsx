  ...
  return <div className="space-y-4">
    ...
    <div className="max-h-[calc(100dvh-15rem)] overflow-auto rounded-2xl border border-[var(--crm-border)] bg-[var(--crm-surface)]">
      <table className="min-w-[1100px] w-full border-collapse text-sm">
        ...
        <tbody>{TIME_SLOTS.map((slot) => <tr key={slot} className="align-top"><th className="sticky left-0 z-[1] border-b border-r border-[var(--crm-border)] bg-[var(--crm-surface)] px-3 py-2 text-left text-xs font-bold text-zinc-300">{slot}</th>{days.map((day) => { const slotStart = dayStamp(day, slot); const slotEnd = slotStart + 30 * 60_000; return <td key={day.toISOString()} className="border-b border-r border-[var(--crm-border)] p-1.5"><div className="space-y-1">{visibleTasks.map((task) => { const assigned = (activities ?? []).filter((activity) => activity.taskId === task._id && activity.startAt < slotEnd && activity.endAt > slotStart); return assigned.length ? assigned.map((activity) => <button key={activity._id} type="button" disabled={!canUpdate} onClick={() => openEdit(day, activity)} className="block w-full rounded-md bg-emerald-100 px-2 py-1 text-left text-[11px] font-semibold text-emerald-900 transition hover:bg-emerald-200 hover:shadow-sm dark:bg-emerald-500/25 dark:text-emerald-100 dark:hover:bg-emerald-500/40">{task.name} · {activity.workerName}</button>) : <button key={task._id} type="button" disabled={!canCreate} onClick={() => openNew(day, slot, task._id)} className="group flex w-full items-center justify-between rounded-md bg-slate-100 px-2 py-1 text-left text-[10px] font-medium text-slate-700 transition hover:bg-slate-200 hover:text-slate-950 hover:shadow-sm dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700 dark:hover:text-white"><span>{task.name}</span><Plus className="h-3 w-3 shrink-0 opacity-0 transition-opacity group-hover:opacity-100" /></button>; })}</div></td>; })}</tr>)}
