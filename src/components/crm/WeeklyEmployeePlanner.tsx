...
export function WeeklyEmployeePlanner({ site }: { site: "60" | "76" }) {
  ...
  const openNew = (day: Date, slot: string) => { const endAt = addMinutes(new Date(dayStamp(day, slot)), 30); setEditing({ day }); setTaskId(""); setWorkerId(""); setStart(slot); setEnd(format(endAt, "HH:mm")); };
  ...
  return <div className="space-y-4">
    ...
    <div className="max-h-[calc(100dvh-15rem)] overflow-auto rounded-2xl border border-[var(--crm-border)] bg-[var(--crm-surface)]">
      <table className="min-w-[1100px] w-full border-collapse text-sm">
        ...
        <tbody>{TIME_SLOTS.map((slot) => <tr key={slot} className="align-top"><th className="sticky left-0 z-[1] border-b border-r border-[var(--crm-border)] bg-[var(--crm-surface)] px-3 py-2 text-left text-xs font-bold text-zinc-300">{slot}</th>{days.map((day) => { const slotStart = dayStamp(day, slot); const slotEnd = slotStart + 30 * 60_000; return <td key={day.toISOString()} className="border-b border-r border-[var(--crm-border)] p-1.5"><div className="space-y-1">{visibleTasks.map((task) => { const assigned = (activities ?? []).filter((activity) => activity.taskId === task._id && activity.startAt < slotEnd && activity.endAt > slotStart); return <div key={task._id} className={assigned.length ? "rounded-md bg-orange-500/15 px-2 py-1" : "rounded-md bg-zinc-800/40 px-2 py-1 text-zinc-600"}>{assigned.length ? assigned.map((activity) => <button key={activity._id} type="button" disabled={!canUpdate} onClick={() => openEdit(day, activity)} className="block w-full text-left text-[11px] font-semibold text-orange-300">{task.name} · {activity.workerName}</button>) : <span className="block text-[10px]">{task.name}</span>}</div>; })}{canCreate ? <button type="button" onClick={() => openNew(day, slot)} className="flex w-full items-center justify-center gap-1 rounded-md border border-dashed border-zinc-700 py-1 text-[11px] text-zinc-500 hover:border-orange-400 hover:text-orange-300"><Plus className="h-3 w-3" />Salarié</button> : null}</div></td>; })}</tr>)}
