...
export function WeeklyEmployeePlanner({ site }: { site: "60" | "76" }) {
  ...
  const [editing, setEditing] = useState<{ workerId?: Id<"polyvalentWorkers">; day: Date; activityId?: Id<"polyvalentActivities">; taskId: Id<"polyvalentTasks"> } | null>(null);
  ...
  const openNew = (activityTaskId: Id<"polyvalentTasks">, day: Date) => { setEditing({ day, taskId: activityTaskId }); setTaskId(""); setStart("09:00"); setEnd("12:00"); };
  const openEdit = (day: Date, activity: NonNullable<typeof activities>[number]) => { setEditing({ workerId: activity.workerId, day, activityId: activity._id, taskId: activity.taskId }); setTaskId(activity.workerId ?? ""); setStart(format(new Date(activity.startAt), "HH:mm")); setEnd(format(new Date(activity.endAt), "HH:mm")); };
  async function save() {
    if (!editing || !taskId) return; setSaving(true);
    try {
      const payload = { taskId: editing.taskId, workerId: taskId as Id<"polyvalentWorkers">, startAt: dayStamp(editing.day, start), endAt: dayStamp(editing.day, end) };
  ...
  return <div className="space-y-4">
    ...
    <div className="max-h-[calc(100dvh-15rem)] overflow-auto rounded-2xl border border-[var(--crm-border)] bg-[var(--crm-surface)]">
      <table className="min-w-[1100px] w-full border-collapse text-sm">
        <thead className="sticky top-0 z-10 bg-[var(--crm-surface-2)] text-zinc-400 shadow-[0_2px_8px_rgba(0,0,0,0.22)]"><tr><th className="sticky left-0 z-20 min-w-56 border-b border-r border-[var(--crm-border)] bg-[var(--crm-surface-2)] px-4 py-3 text-left">Tâche · heures</th>{days.map((day, index) => <th key={day.toISOString()} className="min-w-40 border-b border-r border-[var(--crm-border)] px-3 py-3 text-left"><span className="block font-bold text-zinc-200">{DAYS[index]} <span className="text-brand-300">{durationLabel(dayTotals[index])}</span></span><span className="text-xs">{format(day, "d MMM", { locale: fr })}</span></th>)}<th className="border-b px-3 py-3">Hebdo</th></tr></thead>
        <tbody>{visibleTasks.map((task) => { const weekly = days.reduce((total, day) => total + (activities ?? []).filter((activity) => activity.taskId === task._id && format(new Date(activity.startAt), "yyyy-MM-dd") === format(day, "yyyy-MM-dd")).reduce((sum, activity) => sum + hours(activity.startAt, activity.endAt), 0), 0); return <tr key={task._id} className="align-top"><th className="sticky left-0 z-[1] border-b border-r border-[var(--crm-border)] bg-[var(--crm-surface)] px-4 py-3 text-left"><span className="block font-semibold text-zinc-100">{task.name}</span></th>{days.map((day) => { const cell = (activities ?? []).filter((activity) => activity.taskId === task._id && format(new Date(activity.startAt), "yyyy-MM-dd") === format(day, "yyyy-MM-dd")); return <td key={day.toISOString()} className="border-b border-r border-[var(--crm-border)] p-2"><div className="space-y-1.5">{cell.map((activity) => <button key={activity._id} type="button" disabled={!canUpdate} onClick={() => openEdit(day, activity)} className="block w-full rounded-lg bg-orange-500/15 px-2 py-1.5 text-left hover:bg-orange-500/25"><span className="block text-xs font-bold text-orange-300">{activity.workerName}</span><span className="flex items-center gap-1 text-[11px] text-zinc-300"><Clock3 className="h-3 w-3" />{format(new Date(activity.startAt), "HH:mm")} – {format(new Date(activity.endAt), "HH:mm")}</span></button>)}{canCreate ? <button type="button" onClick={() => openNew(task._id, day)} className="flex w-full items-center justify-center gap-1 rounded-lg border border-dashed border-zinc-700 py-1.5 text-xs text-zinc-500 hover:border-orange-400 hover:text-orange-300"><Plus className="h-3.5 w-3.5" />Salarié</button> : null}</div></td>; })}<td className="border-b px-3 py-3 text-center font-bold text-brand-300">{durationLabel(weekly)}</td></tr>; })}
