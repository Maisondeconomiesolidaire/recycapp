...
export function WeeklyEmployeePlanner({ site }: { site: "60" | "76" }) {
  ...
  const [editing, setEditing] = useState<{ workerId: Id<"polyvalentWorkers">; day: Date; activityId?: Id<"polyvalentActivities">; taskId?: Id<"polyvalentTasks">; start?: string; end?: string } | null>(null);
  ...
  const openNew = (workerId: Id<"polyvalentWorkers">, day: Date) => { setEditing({ workerId, day }); setTaskId(visibleTasks[0]?._id ?? ""); setStart("09:00"); setEnd("12:00"); };
  const openEdit = (workerId: Id<"polyvalentWorkers">, day: Date, activity: NonNullable<typeof activities>[number]) => { setEditing({ workerId, day, activityId: activity._id, taskId: activity.taskId, start: format(new Date(activity.startAt), "HH:mm"), end: format(new Date(activity.endAt), "HH:mm") }); setTaskId(activity.taskId); setStart(format(new Date(activity.startAt), "HH:mm")); setEnd(format(new Date(activity.endAt), "HH:mm")); };
  async function save() {
    if (!editing || !taskId) return; setSaving(true);
    try {
      const payload = { taskId: taskId as Id<"polyvalentTasks">, workerId: editing.workerId, startAt: dayStamp(editing.day, start), endAt: dayStamp(editing.day, end) };
  ...
  return <div className="space-y-4">
    ...
    <div className="overflow-auto rounded-2xl border border-[var(--crm-border)] bg-[var(--crm-surface)]">
      <table className="min-w-[1100px] w-full border-collapse text-sm">
        <thead className="sticky top-0 z-10 bg-[var(--crm-surface-2)] text-zinc-400"><tr><th className="sticky left-0 z-20 min-w-56 border-b border-r border-[var(--crm-border)] bg-[var(--crm-surface-2)] px-4 py-3 text-left">Salarié · heures</th>{days.map((day, index) => <th key={day.toISOString()} className="min-w-40 border-b border-r border-[var(--crm-border)] px-3 py-3 text-left"><span className="block font-bold text-zinc-200">{DAYS[index]}</span><span className="text-xs">{format(day, "d MMM", { locale: fr })}</span></th>)}<th className="border-b px-3 py-3">Hebdo</th></tr></thead>
        <tbody>{visibleWorkers.map((worker) => { const weekly = days.reduce((total, day) => total + (activitiesByCell.get(`${worker._id}-${format(day, "yyyy-MM-dd")}`) ?? []).reduce((sum, activity) => sum + hours(activity.startAt, activity.endAt), 0), 0); return <tr key={worker._id} className="align-top"><th className="sticky left-0 z-[1] border-b border-r border-[var(--crm-border)] bg-[var(--crm-surface)] px-4 py-3 text-left"><span className="block font-semibold text-zinc-100">{worker.firstName} {worker.lastName}</span><span className="text-xs font-normal text-zinc-500">{worker.monthlyHours ? `${durationLabel(worker.monthlyHours / 4)} prévues` : "Heures à définir"}</span></th>{days.map((day) => { const cell = activitiesByCell.get(`${worker._id}-${format(day, "yyyy-MM-dd")}`) ?? []; return <td key={day.toISOString()} className="border-b border-r border-[var(--crm-border)] p-2"><div className="space-y-1.5">{cell.map((activity) => <button key={activity._id} type="button" disabled={!canUpdate} onClick={() => openEdit(worker._id, day, activity)} className="block w-full rounded-lg bg-orange-500/15 px-2 py-1.5 text-left hover:bg-orange-500/25"><span className="block text-xs font-bold text-orange-300">{activity.taskName}</span><span className="flex items-center gap-1 text-[11px] text-zinc-300"><Clock3 className="h-3 w-3" />{format(new Date(activity.startAt), "HH:mm")} – {format(new Date(activity.endAt), "HH:mm")}</span></button>)}{canCreate ? <button type="button" onClick={() => openNew(worker._id, day)} className="flex w-full items-center justify-center gap-1 rounded-lg border border-dashed border-zinc-700 py-1.5 text-xs text-zinc-500 hover:border-orange-400 hover:text-orange-300"><Plus className="h-3.5 w-3.5" />Tâche</button> : null}</div></td>; })}<td className="border-b px-3 py-3 text-center font-bold text-brand-300">{durationLabel(weekly)}</td></tr>; })}
    ...
    <Modal open={Boolean(editing)} onClose={() => setEditing(null)} title={editing?.activityId ? "Modifier la tâche" : "Ajouter une tâche"}><div className="space-y-4"><Field label="Tâche"><Select value={taskId} onChange={(event) => setTaskId(event.target.value)}><option value="">Choisir une tâche</option>{visibleTasks.map((task) => <option key={task._id} value={task._id}>{task.name}</option>)}</Select></Field><div className="grid grid-cols-2 gap-3"><Field label="Début"><Input type="time" value={start} onChange={(event) => setStart(event.target.value)} /></Field><Field label="Fin"><Input type="time" value={end} onChange={(event) => setEnd(event.target.value)} /></Field></div><div className="flex justify-between gap-2">{editing?.activityId && canDelete ? <Button variant="outline" onClick={() => { void deleteActivity({ id: editing.activityId! }); setEditing(null); }}><Trash2 className="h-4 w-4" />Supprimer</Button> : <span /> }<Button disabled={saving || !taskId} onClick={() => void save()}>{saving ? "Enregistrement…" : "Enregistrer"}</Button></div></div></Modal>
